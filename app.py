...
class ToDo:
    def __init__(self):
        ...
        self.commands = [
            ...
            },
            {
                "name": "change",
                "func": self.change,
                "text": "Shows all to do items you can change"
            },
            {
                "name": "exit",
                "func": self.close,
    ...
    def list_all(self):
        ...
        for index, item in enumerate(data, 1):
            if item["done"]:
                print(f"{c}{'{:03d}'.format(index)} {re}{dim}{item['date']} {item['time']} {res}{c}| [x] {re}{item['todo']}")
            else:
    ...
    def change(self):
        ...
        # Input for entering number of list to change to do state
        to_change = input(f"{y}[+] Enter number to change: {re}")

        # Check which state is currently active
        # Then change True to False or False to True
        if data[int(to_change) - 1]["done"]:
            data[int(to_change) - 1]["done"] = False
            print(f"{c}[*] {re}Item {c}{to_change}{re} changed form {c}True{re} to {c}False{re}.")
        else:
            data[int(to_change) - 1]["done"] = True
            print(f"{c}[*] {re}Item {c}{to_change}{re} changed form {c}False{re} to {c}True{re}.")
